...
这只是下一阶段 `/grill-with-docs` 的讨论输入，不是最终决定：
...
1. **收录 7 项还是 8 项**：当前官方仓库已经新增 `x-agent-intelligence`，建议默认按当前快照收录 8 项，但需要用户确认。
2. **技能包归属场景**：该包横跨内容、学习、研究、知识管理和情报监控，现有 Skills123 每个技能包只能归属单一 scenario；需要决定放在“研究”，还是先扩展跨场景展示。
...
进入 `/grill-with-docs`，优先确认四个决定：

1. 按当前官方快照收录 8 项，还是固定为最初 README 的 7 项。
2. “DAIR Academy 技能包”在首页归属哪个 scenario。
3. 第一版只做共享工作区，还是同时扩展详情模块以展示安装、依赖、环境变量、产物和许可证。
4. 是否先落实架构审查中建议的“深化技能清单 module”，再接入第三个技能包。
