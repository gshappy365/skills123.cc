...
1. 21 项全部收录，还是先收录 `/ray` 主路由加一组核心成员。
2. 技能包归属“开发”“研究”“内容”还是扩展为跨场景技能包。
3. 包内分组是否沿用官方 8 条线，还是按 Skills123 用户任务重新策展。
4. 第一版是否展示每个成员的验证、依赖、输出、风险和外部连接信息。
5. 安装按钮复制整包命令，还是成员命令与整包安装命令同时提供。
6. 是否采用固定 commit 快照和手动同步，避免运行时调用 GitHub API。
